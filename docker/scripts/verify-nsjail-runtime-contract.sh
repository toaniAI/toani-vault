...
require_grep "ENV PLAYWRIGHT_BROWSERS_PATH=/ms-playwright" "$RUNTIME_DOCKERFILE"
require_grep 'mkdir -p /opt/credbridge-browser-runtime "$PLAYWRIGHT_BROWSERS_PATH"' "$RUNTIME_DOCKERFILE"
require_grep 'chmod -R a+rX /opt/credbridge-browser-runtime "$PLAYWRIGHT_BROWSERS_PATH"' "$RUNTIME_DOCKERFILE"
