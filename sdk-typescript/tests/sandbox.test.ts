...
describe('SandboxService', () => {
  ...
  describe('获取单个会话', () => {
    it('应该获取会话详情', async () => {
      ...
      expect(client.get).toHaveBeenCalledWith('/sandbox/sessions/session-123', undefined);
    });
    ...
    it('应该在会话不存在时抛出错误', async () => {
      ...
      await expect(service.getSession('nonexistent')).rejects.toThrow(CredBridgeError);
    });
  });
  ...
  describe('快捷方法', () => {
    it('navigate 应该调用 executeOperation', async () => {
      const executeSpy = vi.spyOn(service, 'executeOperation').mockResolvedValue({
        operationId: 'op-123',
        status: OperationStatus.Success,
        executionTimeMs: 500,
      });
    ...
    it('fill 应该调用 executeOperation', async () => {
      ...
      expect(executeSpy).toHaveBeenCalledWith(
        'session-123',
        { operationType: OperationType.Fill, selector: '#username', value: 'user@example.com' },
        undefined
      );
    });
    ...
    it('executeScript 应该调用 executeOperation', async () => {
      ...
      const result = await service.executeScript('session-123', 'return document.title;');

      expect(executeSpy).toHaveBeenCalledWith(
        'session-123',
        { operationType: OperationType.ExecuteScript, script: 'return document.title;' },
        undefined
      );
    ...
    it('getAttribute 应该调用 executeOperation', async () => {
      ...
      const result = await service.getAttribute('session-123', 'a.link', 'href');
  ...
  describe('waitForStatus', () => {
    it('应该在状态匹配时立即返回', async () => {
      ...
      const result = await service.waitForStatus('session-123', SessionStatus.Running);
