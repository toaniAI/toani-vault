...
describe('CredBridgeClient', () => {
  ...
  describe('Token 管理', () => {
    ...
    it('应该检测 Token 是否过期', () => {
      // 使用已过期 Token（exp 在过去）
      const expiredToken = 'v4.local.eyJzdWIiOiJ0ZW5hbnQxOnVzZXIxIiwiZXhwIjoxNzAwMDAwMDAwLCJpYXQiOjE3MDAwMDAwMDAsImp0aSI6InRva2VuMTIzIiwic2NvcGUiOiJjcmVkZW50aWFsOnJlYWQifQ.signature';
  ...
  describe('事件系统', () => {
    ...
    it('应该能够取消事件监听', () => {
      ...
      // 验证监听器已被移除
      expect(client['eventListeners'].get(SdkEventType.RequestStart)?.has(listener)).toBe(false);
    });
  });
  ...
  describe('HTTP 请求', () => {
    ...
    it('应该在 404 时抛出 NotFound 错误', async () => {
      ...
      await expect(client.get('/credentials/nonexistent')).rejects.toThrow(CredBridgeError);
      await expect(client.get('/credentials/nonexistent')).rejects.toMatchObject({
  ...
  describe('重试逻辑', () => {
    ...
    it('应该在 skipRetry 时跳过重试', async () => {
      ...
      await expect(
        client.get('/test', { skipRetry: true })
      ).rejects.toThrow();
