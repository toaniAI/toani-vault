...
async function main() {
  ...
  try {
    ...
    if (error instanceof CredBridgeError) {
      if (error.code === CredBridgeErrorCode.InsufficientScope ||
  ...
  async function safeOperation<T>(
    ...
    try {
      ...
      if (error instanceof CredBridgeError) {
        ...
        const message = errorMessages[error.code] || `${operationName} 失败: ${error.message}`;
  ...
  // 使用通用错误处理函数
  await safeOperation(
    () => client.credentials.get('invalid-id'),
    '获取凭证'
  );
