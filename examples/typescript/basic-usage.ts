...
async function main() {
  ...
  try {
    ...
    for (const cred of credentials) {
      console.log(
        `   - ${(cred as any).credential_id || (cred as any).credentialId} (${(cred as any).credential_type || (cred as any).credentialType})`,
      );
    }
    ...
    console.log(
      "   服务ID:",
      (credential as any).service_id || credential.serviceId,
    );
    console.log(
      "   类型:",
      (credential as any).credential_type || credential.credentialType,
    );
