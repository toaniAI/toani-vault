...
async function main() {
  ...
  try {
    ...
    if (tokenInfo) {
      ...
      console.log('   颁发时间:', new Date(tokenInfo.issuedAt * 1000).toISOString());
      console.log('   过期时间:', new Date(tokenInfo.expiresAt * 1000).toISOString());
    }
    ...
    // 3. 检查权限
    console.log('\n3. 检查权限...');
    console.log('   是否有 read 权限:', client.token.hasScope('credential:read'));
    console.log('   是否有 decrypt 权限:', client.token.hasScope('credential:decrypt'));
    console.log('   是否有 write 权限:', client.token.hasScope('credential:write'));
    ...
    console.log('   是否有 read+decrypt:', client.token.hasAllScopes(readScopes));
    console.log('   是否有 read+write:', client.token.hasAllScopes(writeScopes));
    console.log('   是否有任一 read/write:', client.token.hasAnyScope(readScopes));
