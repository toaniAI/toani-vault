...
export async function runTokens(config: CliConfig, argv: string[]): Promise<void> {
  ...
  switch (subcommand) {
    case 'create': {
      const expiresIn = options['expires-in'] ? Number(options['expires-in']) : 3600;
      const scopeRaw = options.scope as string | undefined;
      const scopes = scopeRaw ? scopeRaw.split(',').map((s) => s.trim()).filter(Boolean) : ['credential:read'];
    ...
    default:
      throw new Error('Usage: toani tokens <create|list|get|verify|stats|revoke> [options]');
  }
}
