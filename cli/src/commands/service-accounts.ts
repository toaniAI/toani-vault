...
export async function runServiceAccounts(config: CliConfig, argv: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'get': {
      const id = requirePositional(options._[0], 'Usage: toani service-accounts get <id>');
    ...
    case 'update': {
      ...
      const response = await client.patch(`/service-accounts/${id}`, {
        ...
        scope_ceiling:
          typeof options.scope === 'string' ? parseScopes(options.scope) : undefined,
      });
      printResult(response, config.output);
      return;
    }
    case 'token': {
      ...
      if (nested === 'create') {
        ...
        const response = await client.post(`/service-accounts/${serviceAccountId}/tokens`, {
          scopes,
      ...
      if (nested === 'list') {
        ...
        const response = await client.get(`/service-accounts/${serviceAccountId}/tokens`);
        printResult(response, config.output);
        return;
      }
      throw new Error(
