import type { OperationType } from '@toani/vault-sdk';
import type { CliConfig } from '../types/cli.js';
import { printResult } from '../output/print.js';
import { createSdk, parseJsonOption, parseOptions, requireArg } from './common.js';

export async function runSandbox(config: CliConfig, argv: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'execute': {
      ...
      if (!sessionId) throw new Error('Usage: toani sandbox execute <sessionId> --operation-type <type>');
      const operationType = requireArg(options, 'operation-type') as OperationType;
      ...
      const result = await sdk.sandbox.executeOperation(sessionId, {
        ...
        bindings: params.bindings as Record<string, string | { $credential: string }> | undefined,
    ...
    case 'get-operation': {
      ...
      if (!operationId) throw new Error('Usage: toani sandbox get-operation <operationId>');
