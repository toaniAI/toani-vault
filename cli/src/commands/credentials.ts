...
export async function runCredentials(config: CliConfig, argv: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'create': {
      ...
      const credentialType = toCredentialType(requireArg(options, 'credential-type'));
      const plaintextData = parseJsonOption(options, 'data');
      const expiresAt = options['expires-at'] ? Number(options['expires-at']) : undefined;
    ...
    case 'decrypt': {
      const id = options._[0];
      if (!id) throw new Error('Usage: toani credentials decrypt <id> [--reason <text>]');
