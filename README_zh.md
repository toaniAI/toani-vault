...
本仓库现在将 SGX 硬件路径拆成两部分：

- Host 侧运行时桥接：位于 `src/tee/host_runtime.rs`、`src/tee/enclave.rs`、`src/tee/provider.rs`、`src/tee/dcap.rs`、`src/tee/sealing.rs`
- Linux SGX Enclave 构建产物：位于 `sgx-enclave/`，由 `scripts/build-sgx-enclave.sh` 和 `scripts/sign-sgx-enclave.sh` 产出

### 它如何运作

在 `TEE_MODE=hardware` 下，服务不再把模拟 root key 或 report 当作默认生产路径，而是优先走真实 enclave artifact + host runtime bridge：

1. Host 读取 `TEE_ENCLAVE_PATH`
2. `SgxHostRuntime` 加载 signed enclave 共享库
3. Host 通过 ECALL/FFI 获取：
   - Enclave identity
   - report
   - sealing key
   - hardware 路径下的 credential encrypt/decrypt
4. `DcapService` 基于 report 生成和验证 quote
5. 如果 enclave artifact、runtime、report、quote、DCAP 依赖任一环节缺失，`TEE_MODE=hardware` 必须 fail-closed，而不是静默回退到 simulation

当前 macOS 主要用于代码编写和 simulation-safe 验证；真实 SGX 编译、签名、quote、硬件测试需要在 Linux SGX runner 或 TEE Docker 中执行。
...
### Drone SGX Runner 部署流程

当前 `.drone.yml` 中的 `backend-hardware-attestation` pipeline 会在 SGX runner 上执行以下步骤：
...
项目内置了专用镜像定义 `docker/sgx/Dockerfile`。它不用于正常业务运行镜像，只用于 SGX build/sign/compile-check。
...
- [SGX Runner 执行手册](docs/07-开发者指南/测试策略/SGX_RUNNER_RUNBOOK.md)
- [SGX Build Image 说明](docker/sgx/README.md)
- [SGX Enclave Skeleton 说明](sgx-enclave/README.md)
